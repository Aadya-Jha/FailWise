...
FailWise is a retrieval-augmented search system for engineering incident postmortems. It combines metadata-aware filtering with semantic retrieval to answer natural-language questions over real outage reports from companies such as Cloudflare, AWS, GitHub, Stripe, and Discord.
...
5. Queries execute through hybrid retrieval:
   - metadata filtering
...
Postmortems are segmented by semantic sections such as timeline, root cause, resolution, and learnings rather than fixed token windows. Each chunk preserves a single semantic unit, improving retrieval precision and contextual relevance during search.

### Hybrid Retrieval

FailWise applies metadata filtering before semantic retrieval. Structured constraints such as company, severity, or category narrow the search space before vector similarity ranking executes, improving retrieval quality for targeted queries.
...
Postmortems are sourced from public engineering blogs and the community-maintained `danluu/post-mortems` repository.
...
* Evaluate distributed vector databases for cloud-native persistence
* Add incident similarity search for related outage discovery
* Integrate Slack-based querying workflows for operational teams
